...
class Render:
    ...
    def clear_all(cons, entities):
        for entity in entities:
            Render.clear_entity(cons, entity)
    ...
    def render_all(cons, bars_cons, player, game_map, entities, screen_width, screen_height, colors, fov_mode,
        ...
        for entity in sorted(entities, key=lambda ent: ent.render_order.value):
            Render.draw_entity(cons, game_map, entity, fov_mode, fov_map)
        ...
        Render.render_bar(bars_cons, 0, 0, 26, 'HP', player.stats.hp, player.stats.max_hp,
                          tc.light_red, tc.darker_red)
        defense = player.stats.defense
        if player.stats.armour:
            defense += player.stats.armour.item.defense

        Render.render_bar(bars_cons, 0, 1, 26, 'DEFENSE', defense, player.stats.max_defense,
